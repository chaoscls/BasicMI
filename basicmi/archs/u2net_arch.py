...
class RSU7(nn.Module):#UNet07DRES(nn.Module):
    ...
    def forward(self,x):
        ...
        hx3d = self.rebnconv3d(torch.cat((hx3dup,hx2),1))
        hx3dup = _upsample_like(hx3d,hx1)

        hx1d = self.rebnconv1d(torch.cat((hx3dup,hx1),1))
...
class RSU4F(nn.Module):#UNet04FRES(nn.Module):
    ...
    def forward(self,x):
        ...
        hx3d = self.rebnconv3d(torch.cat((hx4,hx3),1))
        hx3d = self.rebnconv3d(torch.cat((hx3d,hx2),1))
        hx1d = self.rebnconv1d(torch.cat((hx3d,hx1),1))
...
class U2NET(nn.Module):
    ...
    def forward(self,x):
        ...
        hx3d = self.stage3d(torch.cat((hx3dup,hx2),1))
        hx3dup = _upsample_like(hx3d,hx1)

        hx1d = self.stage1d(torch.cat((hx3dup,hx1),1))
...
class U2NETP(nn.Module):
    ...
    def __init__(self,in_channels=3,out_channels=1):
        ...
        self.stage3d = RSU6(128,16,64)
        self.stage1d = RSU7(128,16,64)

        self.side1 = nn.Conv3d(64,out_channels,3,padding=1)
        self.side2 = nn.Conv3d(64,out_channels,3,padding=1)
        self.side3 = nn.Conv3d(64,out_channels,3,padding=1)
        self.side4 = nn.Conv3d(64,out_channels,3,padding=1)
        self.side5 = nn.Conv3d(64,out_channels,3,padding=1)
        self.side6 = nn.Conv3d(64,out_channels,3,padding=1)

        self.outconv = nn.Conv3d(6*out_channels,out_channels,1)
    ...
    def forward(self,x):
        ...
        # return F.sigmoid(d0), F.sigmoid(d1), F.sigmoid(d2), F.sigmoid(d3), F.sigmoid(d4), F.sigmoid(d5), F.sigmoid(d6)
        return (d0, d1, d2, d3, d4, d5, d6) if self.training else d0